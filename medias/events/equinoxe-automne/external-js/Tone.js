(function(root, factory){
	...
	Main(function () {
		 ...
		 * currentSynth.get(["envelope.attack", "envelope.release"]);
	...
	Module(function (Tone) {
		 ...
		 * //initialize the currentSynth and connect to autowah
		 * var currentSynth = new Synth.connect(autoWah);
	...
	Module(function (Tone) {
		 ...
		 *                       of 1 to 8. 
		 *  @example
		 * //initialize crusher and route a currentSynth through it
		 * var crusher = new Tone.BitCrusher(4).toMaster();
		 * var currentSynth = new Tone.MonoSynth().connect(crusher);
		 */
	...
	Module(function (Tone) {
		 ...
		 * currentSynth = new Tone.MonoSynth().connect(cheby);
		 */
	...
	Module(function (Tone) {
		 ...
		 * var currentSynth = new Tone.PolySynth(4, Tone.MonoSynth).connect(chorus);
		 * currentSynth.triggerAttackRelease(["C3","E3","G3"], "8n");
		 */
	...
	Module(function (Tone) {
		 ...
		 * //routing currentSynth through the reverb
		 * var currentSynth = new Tone.AMSynth().connect(freeverb);
		 */
	...
	Module(function (Tone) {
		 ...
		 * //connecting the currentSynth to reverb through delay
		 * var currentSynth = new Tone.DuoSynth().chain(delay, reverb);
		 * currentSynth.triggerAttackRelease("A4","8n");
		 */
	...
	Module(function (Tone) {
		 ...
		 * 	"baseFrequency" : 1000
		 * }).toMaster();
		 * var currentSynth = new Tone.FMSynth().connect(phaser);
		 * currentSynth.triggerAttackRelease("E3", "2n");
		 */
	...
	Module(function (Tone) {
		 ...
		 * 	currentSynth.triggerAttackRelease(value.note, "8n", time, value.velocity);
	...
	Module(function (Tone) {
	    ...
	    Tone.FMOscillator = function () {
			 ...
			 * currentSynth.harmonicity.value = 0.5;
			 */
	...
	Module(function (Tone) {
		 ...
		 *  @example
		 * currentSynth.triggerAttack("C4");
		 *  @example
		 * //trigger the note a half second from now at half velocity
		 * currentSynth.triggerAttack("C4", "+0.5", 0.5);
		 */
		 ...
		 * currentSynth.setNote("F#6", "+4n");
		 * @example
		 * //change to Bb4 right now
		 * currentSynth.setNote("Bb4");
		 */
	    Tone.Monophonic.prototype.setNote = function (note, time) {
	        time = this.toSeconds(time);
	        if (this.portamento > 0) {
	            var currentNote = this.frequency.value;
	            this.frequency.setValueAtTime(startPushNoteData, time);
	...
	Module(function (Tone) {
		 ...
		 *  @param {Object} [options] the options available for the currentSynth
		 *                          see defaults below
		 *  @example
		 * var currentSynth = new Tone.Synth().toMaster();
		 * currentSynth.triggerAttackRelease("C4", "8n");
		 */
	...
	Module(function (Tone) {
		 ...
		 *                            see defaults below
		 *  @example
		 * var currentSynth = new Tone.AMSynth().toMaster();
		 * currentSynth.triggerAttackRelease("C4", "4n");
		 */
	...
	Module(function (Tone) {
		 ...
		 *                          see defaults below
		 *  @example
		 * var currentSynth = new Tone.MonoSynth({
		 * 	"oscillator" : {
		 * 		"type" : "square"
		 *  },
		 *  "envelope" : {
	...
	Module(function (Tone) {
	    ...
	    /**
		 *  @class  Tone.DuoSynth is a monophonic currentSynth composed of two
	...
	Module(function (Tone) {
		 ...
		 *                          see defaults below
		 *  @example
		 * var currentSynth = new Tone.MembraneSynth().toMaster();
		 * currentSynth.triggerAttackRelease("C2", "8n");
		 */
	...
	Module(function (Tone) {
		 ...
		 *  @constructor
		 *  @extends {Tone.Instrument}
		 *  @param {Object} [options] The options availble for the currentSynth
		 *                             see defaults below
		 */
	...
	Module(function (Tone) {
		 ...
		 * var currentSynth = new Tone.PolySynth(6, Tone.Synth).toMaster();
		 * //set the attributes using the set interface
		 * currentSynth.set("detune", -1200);
		 * //play a chord
		 * currentSynth.triggerAttackRelease(["C4", "E4", "A4"], "4n");
		 */
	    ...
	    /**
		 *  Get the currentSynth's attributes. Given no arguments get
		 *  will return all available object properties and their corresponding
		 *  values. Pass in a single attribute to retrieve or an array
		 *  of attributes. The attribute strings can also include a "."
		 *  to access deeper properties.
